...
public interface Task<T> {
	 ...
	 * and conquer algorithm</a>.
	 * <br>
	 * <br>
	 * (1) <i>if(getStatus() == Task.Status.DECOMPOSE)</i>
	 * <br>
	 * <br>
	 * Then at runtime this method should divide the existing task
	 * into smaller subtasks, pack the subtasks in a {@link api.Result Result}
	 * object and return them. The returned subtasks are consumed by a
	 ...
	 * <br>
	 * <br>
	 * (2) <i>if(getStatus() == Task.Status.COMPOSE)</i>
	 * <br>
	 * <br>
	 * Then at runtime this method should compose results of smaller subtasks that were earlier
	 * generated by this task during the 'Divide' phase, into a single value
	 * that can be consumed by the parent of this task in the overall recursion
	 * tree. See {@link #putValues(List)} and {@link #getValues()} to know
	 * how values can be passed to setup the 'Conquer' phase of execution.
	 * 
	 ...
	 * @return values
	 *            A java.util.List containing results obtained from the smaller
	 *            subtasks that were originally generated by the same task and
	 *            populated using {@link #putValues(List)} method.
	 */
